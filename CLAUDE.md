...
- `functions` — Express REST API deployed as a Cloud Function (2nd gen, `europe-west1`) plus a scheduled daily job. Bundled with esbuild (`lib/index.js`); `@tms/shared` is inlined. Firestore via Admin SDK only; all authorisation is server-side.
